...
//     console.log(111);
//     res.send('这是注册路由');
//     pool.query('SELECT *  FROM Airbnb_user',[],(err,result)=>{
//         console.log(222);
//         if(err){throw err};
//         //if(result.length>0){
//             console.log(333);
...
//     //执行sql
//     pool.query('SELECT user_name,user_pwd  FROM Airbnb_user  WHERE uname=? && upwd=?',[obj.user_name,obj.user_pwd],(err,result)=>{
//         if(err){throw err};
//         if(result.length>0){
...
router.get("/", (req, res) => {
  ...
  var user_pwd=req.query.user_pwd;
   if(!user_name){
    ...
    var sql = `SELECT * FROM Airbnb_user WHERE user_name=? && user_pwd=?`
...
router.get("/reg",(req,res)=>{
    ...
    var sql="INSERT INTO Airbnb_user SET ?";
...
router.get('/selName',(req,res)=>{
    var user_name=req.query.user_name;
    var sql="SELECT * FROM Airbnb_user WHERE user_name=?";
